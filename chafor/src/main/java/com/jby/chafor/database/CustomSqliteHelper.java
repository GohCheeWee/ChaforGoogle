package com.jby.chafor.database;
...
import com.jby.chafor.exportFeature.category.searchFeature.ExportCategorySearchCategoryObject;
import com.jby.chafor.exportFeature.category.searchFeature.ExportCategorySearchSubCategoryObject;
import com.jby.chafor.exportFeature.subcategory.subcategory.SubCategoryObject;
import com.jby.chafor.exportFeature.subcategory.subcategory.takeAction.TakeActionObject;
import com.jby.chafor.others.CSVWriter;
...
public class CustomSqliteHelper extends SQLiteOpenHelper {
   ...
   public CustomSqliteHelper(Activity context) {
    ...
    public ArrayList<ExportCategoryListViewObject> fetchAllCategory(int page, ArrayList<ExportCategoryListViewObject> currentArrayList) {
        SQLiteDatabase db = this.getReadableDatabase();

        double start;
        double limit = 10.0;
        String userId = SharedPreferenceManager.getUserID(context);
        int totalCategoryRow = countCategoryRow(userId);
        ...
        if(page <= page_limit) {
            ...
            String sql = "SELECT tb_export_category.category_id, tb_export_category.category_name, COUNT(tb_export_sub_category.sub_category_id) FROM " + TB_CATEGORY +
                    ...
                    " WHERE tb_export_category.user_id=" + userId +
    ...
    private int countCategoryRow(String userID){
        SQLiteDatabase db = this.getReadableDatabase();

        String sql = "SELECT category_id FROM "+ TB_CATEGORY + " WHERE user_id=" + userID;
...
//        if failed
    ...
    public ArrayList<ExportCategorySearchSubCategoryObject> searchAllSubCategoryFromCategory(ArrayList<ExportCategorySearchSubCategoryObject> currentArrayList, String keyword) {
        SQLiteDatabase db = this.getReadableDatabase();
        String userID = SharedPreferenceManager.getUserID(context);
            ...
            String sql = "SELECT tb_export_sub_category.barcode, tb_export_sub_category.quantity, tb_export_category.category_name," +
                    ...
                    " WHERE tb_export_category.user_id=" + userID + " AND (tb_export_sub_category.barcode LIKE '%" + keyword + "%')"+
    ...
    public ArrayList<ExportCategorySearchCategoryObject> searchAllCategoryByQuery(ArrayList<ExportCategorySearchCategoryObject> currentArrayList, String keyword) {
        SQLiteDatabase db = this.getReadableDatabase();
        String userID = SharedPreferenceManager.getUserID(context);
            ...
            String sql = "SELECT tb_export_category.category_id, tb_export_category.category_name, COUNT(tb_export_sub_category.sub_category_id) FROM "
                    ...
                    " ON tb_export_category.category_id = tb_export_sub_category.category_id" +
                    " WHERE tb_export_category.user_id="+ userID +" AND (tb_export_category.category_name LIKE '%" + keyword + "%')" +
                    " GROUP BY tb_export_category.category_name" +
                    " ORDER BY tb_export_category.category_id DESC";
    ...
    public int saveSubCategory(int count, String categoryID, String barcode, int quantity)
    {
...
//            check this record existed or not
            int checkRecordAvailabilityInOthers = checkSubCategoryExistedInOther(categoryID, barcode);
//            if exist
...
//            if reminder = off or record is not exist in other category then proceed to here
    ...
    private int checkSubCategoryExistedInOther(String categoryID, String barcode){
        SQLiteDatabase db = this.getReadableDatabase();
        String userID = SharedPreferenceManager.getUserID(context);
        ...
        String sql = "SELECT tb_export_sub_category.sub_category_id FROM " + TB_SUB_CATEGORY +
                ...
                " AND tb_export_category.user_id =?";

        Cursor cursor = db.rawQuery(sql,  new String[] { categoryID, barcode, userID });
        int cursorCount = cursor.getCount();
        cursor.close();
        return cursorCount;
    }
    ...
    public int updateSubCategory(int count, String categoryID, String barcode, int quantity, String subCategoryID)
    {
        String reminder = SharedPreferenceManager.getReminder(context);
        int status = 0;
        boolean reminderStatus = false;
...
//            if reminder = off or record is not exist in other category then proceed to here
    ...
    public ArrayList<TakeActionObject> fetchAllExistedRecordFromOther(String categoryID, String barcode, ArrayList<TakeActionObject> arrayList){
        SQLiteDatabase db = this.getWritableDatabase();
        String userID = SharedPreferenceManager.getUserID(context);
        ...
        String sql = "SELECT tb_export_sub_category.sub_category_id, tb_export_sub_category.barcode," +
                ...
                " ON tb_export_sub_category.category_id = tb_export_category.category_id" +
                " WHERE tb_export_sub_category.category_id<>?" +
                " AND tb_export_sub_category.barcode=? AND tb_export_category.user_id=?" +
                " ORDER BY tb_export_sub_category.priority DESC";
    ...
    public void exportFile(View view){
        //        disable the button until process done
...
//      create directory
        ...
        String userID = SharedPreferenceManager.getUserID(context);
        //csv
        if(SharedPreferenceManager.getExportType(context) == 1) exportCsvFile(userID, dir);
        //text file
        else exportTextFile(userID, dir);

        //enable the button
        view.setEnabled(true);
    }
    private void exportCsvFile(String userID, File dir) {
//      create file
        ...
        try
        {
            ...
            Cursor curCSV = db.rawQuery(exportQuery() , new String[] { userID });
    ...
    private void exportTextFile(String userID, File dir){
//      create file
        String fileName = "export_file.txt";
        File file = new File(dir.getAbsolutePath(), fileName);
        try
        {
            SQLiteDatabase db = this.getReadableDatabase();
            ...
            // text file header
            bw.write("Category,Barcode,Quantity,Date Create,Time Create"+ "\r\n");
            while(curCSV.moveToNext())
            {
                //Which column you want to export
                String arrStr =
    ...
    private String exportQuery(){
        return  "SELECT tb_export_category.category_name, tb_export_sub_category.barcode, tb_export_sub_category.quantity, " +
                "tb_export_sub_category.date_create, tb_export_sub_category.time_create" +
                " FROM " + TB_CATEGORY +
                " INNER JOIN " + TB_SUB_CATEGORY +
                " ON tb_export_category.category_id = tb_export_sub_category.category_id" +
                " WHERE tb_export_category.user_id = ?";
    }
